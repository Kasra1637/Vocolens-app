...
function WeekView({
  ...
  const toY = useCallback(
    (val: number) => PAD_Y + innerH - (val / 100) * innerH,
    [innerH],
  );
  const toX = useCallback(
    (i: number) => PAD_X + (i / 6) * innerW,
    [innerW],
  );

  // Interpolate missing days linearly between known values
  ...
  // Build SVG path segments (solid for real entries, dashed for gaps)
  const segments = useMemo(() => {
    ...
    return result;
  }, [days, interpolated, toX, toY]);

  // Area fill path (bottom-closed polygon)
  const areaPath = useMemo(() => {
    const pts = days.map((_, i) => `${toX(i)},${toY(interpolated[i])}`).join(" L ");
    const baseY = PAD_Y + innerH;
    return `M ${toX(0)},${toY(interpolated[0])} L ${pts} L ${toX(6)},${baseY} L ${toX(0)},${baseY} Z`;
  }, [days, interpolated, toX, toY, innerH]);

  const hasAnyEntry = days.some((d) => d.hasEntry);

  const trendConfig = {
    up: { label: `+${trendDelta} improving`, color: "#FFFFFF", Icon: TrendingUp },
    down: { label: `${trendDelta} declining`, color: "rgba(255,255,255,0.7)", Icon: TrendingDown },
    stable: { label: "Stable this week", color: "rgba(255,255,255,0.6)", Icon: Minus },
  }[trend];
  ...
  return (
    <View style={{ paddingHorizontal: 20, paddingBottom: 20 }}>
      ...
      {hasAnyEntry && (
        ...
        >
          <TrendIcon size={13} color={trendConfig.color} strokeWidth={2.5} />
          <Text
            style={{
          ...
          <Svg width={chartWidth} height={CHART_H}>
            ...
            {segments.map((seg, i) => (
              <Path
                key={i}
                d={seg.d}
                stroke={seg.dashed ? "rgba(255,255,255,0.18)" : primaryColor}
      ...
      {/* Peak / valley callout */}
      {hasAnyEntry && peakIdx >= 0 && valleyIdx >= 0 && (
        <View
          style={{
            flexDirection: "row",
            gap: 8,
