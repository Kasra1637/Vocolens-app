...
import useOnboardingStore, { THEME_COLORS, JournalingGainType } from '@/lib/state/onboarding-store';
...
export function JournalingGoalsScreen() {
  ...
  const setSelectedJournalingGain = useOnboardingStore((s) => s.setSelectedJournalingGain);
  const selectedTheme = useOnboardingStore((s) => s.selectedTheme);
  const currentStep = useOnboardingStore((s) => s.currentStep);
  const themeColors = THEME_COLORS[selectedTheme];
  const playClickSound = useClickSound();

  const [selectedGain, setLocalGain] = useState<JournalingGainType | null>(null);
          ...
          <View className="flex-1 px-6 py-3">
            ...
            >
              <Pressable
                ...
                style={{
                  borderWidth: 2,
                  borderColor: selectedGain ? '#FFFFFF' : 'rgba(255, 255, 255, 0.3)',
                  shadowColor: '#000',
