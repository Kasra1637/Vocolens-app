...
function MiniMicIcon({ size, color, filled }: { size: number; color: string; filled?: boolean }) {
...
function LiveDot() {
  ...
  return (
    <Animated.View
      style={[{ width: 5, height: 5, borderRadius: 3, backgroundColor: '#EF4444', marginLeft: 5 }, style]}
    />
  );
}
...
function AnimatedBar({
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      barWidth.value = withTiming(score, { duration: 800, easing: Easing.out(Easing.ease) });
    }, delay);
    return () => clearTimeout(t);
  }, []);
  ...
  return (
    <View style={{ marginBottom: 6 }}>
      ...
      <View style={{ height: 4, borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.1)' }}>
        <Animated.View
          style={[{ height: '100%', borderRadius: 2, backgroundColor: primaryColor }, barStyle]}
        />
      </View>
    </View>
  );
...
export function FreeTrialPreviewScreen() {
  ...
  useEffect(() => {
    const cycle = () => {
      ...
      }, RECORDING_PHASE_DURATION + TRANSITION_DURATION * 2 + ENTRY_PHASE_DURATION);

      return [t1, t2];
    };

    const timers = cycle();
    const totalCycle = RECORDING_PHASE_DURATION + ENTRY_PHASE_DURATION + TRANSITION_DURATION * 4;
        ...
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <View style={{ flex: 1, paddingHorizontal: 24, justifyContent: 'space-between', paddingBottom: 28 }}>
            ...
            >
              ...
              >
                <LinearGradient
                  colors={[themeColors.gradientStart, themeColors.primary, themeColors.secondary]}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 0, y: 1 }}
                  style={{ flex: 1 }}
                >
                  ...
                  {phase === 'recording' && (
                    <Animated.View style={[{ flex: 1 }, recordingPhaseStyle]}>
                      <View style={{ flex: 1, alignItems: 'center', paddingTop: 14, paddingHorizontal: 14 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 8, gap: 20 }}>
                          {/* Pause button */}
                    ...
                    <Animated.View style={[{ flex: 1 }, entryPhaseStyle]}>
                      <View style={{ flex: 1, paddingHorizontal: 12, paddingTop: 10 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                          ...
                          >
                            <ArrowLeft size={12} color="#FFFFFF" strokeWidth={2.5} />
                          </View>
                        </View>
                        ...
                        >
                          ...
                          <AnimatedBar label="Joy" score={65} barOpacity={0.75} delay={400} primaryColor={themeColors.primary} />
                          <AnimatedBar label="Interest" subLabel="anticipation" score={52} barOpacity={0.55} delay={600} primaryColor={themeColors.primary} />
                          <AnimatedBar label="Pensiveness" subLabel="sadness" score={31} barOpacity={0.4} delay={800} primaryColor={themeColors.primary} />
                        </View>
            ...
            <Animated.View entering={FadeInUp.delay(500).duration(600)} style={{ alignItems: 'center' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
                <Check size={14} color="#FFFFFF" strokeWidth={2.5} style={{ marginRight: 6 }} />
                <Text
                  style={{
                    color: '#FFFFFF',
                    fontFamily: 'Inter_400Regular',
...
function AnimatedReflectionText() {
  const reflectionText = "You showed real self-awareness today. Setting boundaries is a sign of growth — keep trusting the process.";
  const [charIndex, setCharIndex] = React.useState(0);
