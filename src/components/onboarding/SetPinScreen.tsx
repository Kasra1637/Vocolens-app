...
import { successHaptic, tapHaptic, errorHaptic, selectHaptic } from '@/lib/haptics';
...
export function SetPinScreen() {
  ...
  const handleComplete = useCallback((pin: string) => {
    if (phase === 'enter') {
      if (isRepeating(pin)) { doShake('PIN cannot be all the same digit'); return; }
      if (isSequential(pin)) { doShake('PIN cannot be sequential like 1234'); return; }
      setFirstPin(pin);
      setDigits('');
      setError('');
      setPhase('confirm');
  ...
  }, [phase, firstPin, doShake, setPin, setPinVerified, setHasCompletedOnboarding, successScale]);
  ...
  const phaseTitle = phase === 'enter' ? 'Set Your PIN' : phase === 'confirm' ? 'Confirm Your PIN' : "You're Locked In!";
  const phaseSubtitle =
    phase === 'enter'
    ...
    <View style={{ flex: 1 }}>
      ...
      <LinearGradient colors={bgColors} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={{ flex: 1 }}>
        ...
        <SafeAreaView style={{ flex: 1 }}>
          ...
          <Pressable
            style={{ flex: 1 }}
            onPress={() => { if (phase !== 'success') inputRef.current?.focus(); }}
          >
            <View
              style={{ flex: 1, alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 24, paddingBottom: 48, paddingTop: 8 }}
              pointerEvents="box-none"
            >
              {/* Top: character + text */}
              <Animated.View entering={FadeInDown.duration(500)} style={{ alignItems: 'center', gap: 16 }}>
                <EmotionalCompanion
              ...
              <Animated.View entering={FadeInDown.delay(100).duration(500)} style={{ alignItems: 'center', gap: 20 }}>
                {phase === 'success' ? (
                  <Animated.View
                    style={[{
                      width: 88,
                  ...
                  <>
                      ...
                      {(['enter', 'confirm'] as Phase[]).map((p) => (
                        <View
                          ...
                          style={{
                            ...
                            paddingVertical: 5,
                            borderRadius: 999,
                            backgroundColor: phase === p ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.08)',
                            borderWidth: 1,
                            borderColor: phase === p ? 'rgba(255,255,255,0.6)' : 'rgba(255,255,255,0.15)',
                          }}
                        >
                          <Text style={{ fontFamily: 'Inter_600SemiBold', fontSize: 12, color: phase === p ? '#FFFFFF' : 'rgba(255,255,255,0.45)' }}>
                    ...
                    {error ? (
                      ...
                      <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 12, color: 'rgba(255,255,255,0.45)', textAlign: 'center' }}>
                        {phase === 'enter' ? 'Avoid sequences (1234) or repeated digits (1111)' : 'Tap the same digits as before'}
                      </Text>
                    )}
                  </>
                )}
              ...
              {phase !== 'success' && (
                <Animated.View entering={FadeInUp.delay(160).duration(500)} style={{ alignItems: 'center', gap: 10 }}>
                  <View style={{
                    ...
                    <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 14, color: 'rgba(255,255,255,0.65)', textAlign: 'center' }}>
                      Use your keypad to enter 4 digits
                    </Text>
                  </View>
                  <Text style={{ fontFamily: 'Inter_400Regular', fontSize: 12, color: 'rgba(255,255,255,0.35)', textAlign: 'center' }}>
