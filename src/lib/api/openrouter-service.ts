...
function parseResponse(result) {
  ...
  const es={happiness:0,sadness:0,anger:0,disgust:0,fear:0,surprise:0,trust:0,anticipation:0};
  if(result.emotionScores)for(const e of ve){const s=Number(result.emotionScores[e]);es[e]=isNaN(s)?0:Math.max(0,Math.min(100,s));}
  ...
  const title=tw.length>=3&&tw.length<=6
    ?tw.map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(" ")
    :tw.length>6
      ?tw.slice(0,6).map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(" ")
      :tw.length>0
        ?tw.map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(" ")
        :"Journal Entry";
  return {
    ...
    emotionScores:es,
    emotionIntensityLabels:buildIntensityLabels(es),
    topics:((result.topics??["reflection"]).slice(0,5)),
    analysis:result.analysis||"Your journal entry has been recorded.",
    reflection:result.recommendation||result.reflection||"Thank you for sharing. Your feelings are valid.",
    ...
    aiTopThreeEmotions:top,
    aiBlendedEmotions:bl,
    aiAmbivalenceFlags:am,
  };
}

export async function analyzeWithOpenRouter(transcript, _a?, personalizationContext?) {
...
export async function generateRecommendation(transcript, primaryEmotion="happiness") {
  ...
  const ev={happiness:"The genuine joy coming through in what you shared is real and worth sitting with.",sadness:"The heaviness you described takes courage to name, and naming it already matters.",anger:"The frustration you felt is entirely understandable — your reaction makes complete sense.",disgust:"The discomfort you described points directly to something you deeply value.",fear:"The worry you named out loud took real courage, and that act alone is significant.",surprise:"The unexpected turn you described caught you off guard, and your response to it shows real self-awareness.",trust:"The quiet confidence running through your words is something worth recognising and building on.",anticipation:"The anticipation you feel shows how much this next step genuinely matters to you."};
