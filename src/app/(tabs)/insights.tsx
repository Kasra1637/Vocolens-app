...
// ── SVG chart builders ──────────────────────────────────────────────────────
// Rendered as inline <svg> inside the report HTML. Any browser (or the OS's
// HTML viewer when the recipient opens the shared file) renders SVG
// natively, so these produce real vector charts in the report — not just
// CSS bars or a table of numbers.
...
<body>
  ...
  <div class="footer">
    ...
    Report period: ${rangeLabel}. This report is confidential and intended for personal health tracking and professional consultation only.<br>
  ...
  // ── Write the report as an HTML file (original approach) ─────────────────
  // Deliberately NOT using expo-print here: it's a native module, so it only
  // works on a build that was compiled with it linked in. Since this file
  // ships via OTA (no new AAB), any device on an older native build would
  // crash calling Print.printToFileAsync(). The HTML + native share sheet
  // approach works on every existing installed build without requiring a
  // new App Store / Play Store upload.
      ...
      {shareToast && (
        <Animated.View
          ...
          style={{
            ...
            left: 24,
            right: 24,
            backgroundColor: "rgba(30,28,48,0.97)",
            borderRadius: 18,
            borderWidth: 1.5,
